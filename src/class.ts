...
class Main {
          ...
          switch (data?.map?.game_state) {
               default:
                    if (data?.player?.team2) {
                         ...
                         this.data = { state: "Main Lobby", largeImageKey: "logo", largeImageText: this.steamName ?? data?.provider.name ?? "Dota 2", startTimestamp: this.data.startTimestamp };
                    }
                    break;
               ...
               case "DOTA_GAMERULES_STATE_HERO_SELECTION":
                    ...
                    this.data.smallImageKey = this.config.displayMini ? "empty_hero" : data.player.team_name;
                    this.data.smallImageText = this.config.displayMini ? undefined : `Team ${this.fixName(data.player.team_name)}`;
                    this.data.largeImageKey = this.config.displayMini ? data.player.team_name : "empty_hero";
                    this.data.largeImageText = this.config.displayMini ? `Team ${this.fixName(data.player.team_name)}` : undefined;
                    break;
               case "DOTA_GAMERULES_STATE_STRATEGY_TIME":
                    this.idleTime = 0;
                    this.data.details = "Pre-Match";
                    this.data.smallImageKey = `${this.fixNameKey(this.config.displayMini ? data.hero.name : `${data.player.team_name}_mini`)}`;
                    this.data.smallImageText = `${this.fixName(this.config.displayMini ? data.hero.name : `team_${data.player.team_name}`)}`;
                    this.data.largeImageKey = `${this.fixNameKey(this.config.displayMini ? data.player.team_name : data.hero.name)}`;
                    this.data.largeImageText = `${this.fixName(this.config.displayMini ? `team_${data.player.team_name}` : data.hero.name)} `;
                    break;
               case "DOTA_GAMERULES_STATE_PRE_GAME":
               case "DOTA_GAMERULES_STATE_GAME_IN_PROGRESS":
               case "DOTA_GAMERULES_STATE_POST_GAME":
                    this.idleTime = 0;
                    this.data.details = `Level ${data.hero.level ?? 0} `;
     ...
     fixNameKey(str: string) {
          str = str.replace("npc_dota_hero_", '');
          str += this.config.displayMini ? "_mini" : '';
          return str;
     }

     fixName(str: string) {
          str = str.replace("npc_dota_hero_", '');
          if (str === "wisp") return "IO";
